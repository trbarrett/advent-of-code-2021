...
let inline dictAddOrUpdateFn key add updateFn (dict : IDictionary<_,_>) =
    if dict.ContainsKey key
    then
        let v = updateFn dict.[key]
        dict.[key] <- v
    else dict.Add(key, add)
...
let astar start goal h =
    ...
    fScore.Add((h start), SortedSet([start]))
...
let part2 (startState : State) =
    ...
    // Correct Answer: 56324 took: 28,188ms (whoa)
